...
namespace Microsoft.Bot.Sample.LuisBot
{
    ...
    {
        private static Dictionary<DateTime, string> roster = new Dictionary<DateTime, string>() {
...
{   new DateTime(   2019    ,   9   ,   9   )   ,   "	Hazz	"    }
        ...
        public class Date
        {
            ...
            public static implicit operator Date(List<object> list)
            {
                var values = list.First() as Dictionary<string, object>;

                var date = new Date()
                {
                    TimeX = values["timex"].ToString(),
                    Type = values["type"].ToString(),
                    Value = DateTime.Parse(values["value"].ToString())
                };
            ...
            {
                return $"TimeX: {TimeX}, Type: {Type}, Value: {Value}";
            }
        }
    }
}